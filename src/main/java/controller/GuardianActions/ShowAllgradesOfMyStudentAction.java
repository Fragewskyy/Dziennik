...
public class ShowAllgradesOfMyStudentAction implements Action {
    ...
    public void executeQuery() throws SQLException {
        ...
        String query  = "SELECT grade, student_id, (select lesson_date from lessons where lessons.lesson_id = grades.lesson_id), (select subject_name from subjects where subjects.subject_id = (select subject_id from lessons where lessons.lesson_id = grades.lesson_id)) from grades where grades.student_id in (select student_id from student where student.guardian_id = " + guardian.guardianId + ");";
